...
export const SCRIPTS = [
  ...
  },
  {
    ...
    covers: "regenerates every UI screenshot by driving the site's own dev server, and records what each frame rendered in docs/.vitepress/screenshots/manifest.json. Replaces the committed frames only after every frame and the budget pass, and rolls the replacement back if a step of it fails",
    cost: null,
  },
  {
    ...
    selfTests: true,
    implements: "docs/.vitepress/scripts/shoot-screenshots.mjs",
    covers: "renders the same flow against the site's dev server and fails when a frame's text, elements, geometry or key styles no longer match the manifest, or an image no longer matches its recorded hash. Writes nothing",
    cost: null,
  },
  {
    name: "og-image",
...
export const RULES = [
  ...
  {
    prefix: "docs/.vitepress/scripts/shoot-screenshots.mjs",
    scripts: ["screenshots", "screenshots:check"],
    why: "One file behind two npm scripts, so the generated one-script rule above cannot describe it: the run that shoots and the check that compares walk the same flow through the site. Both run its --self-test first.",
  },
  {
    prefix: "docs/.vitepress/screenshots/",
    scripts: ["screenshots", "screenshots:check"],
    why: "The fingerprint rules and the manifest they write. The manifest is generated by the screenshot run, never by hand, and the check reads it against the site as it renders now; the rules are a module rather than a check, which is why they live outside scripts/, and the self-test both scripts run first is where each rule is proved able to fail.",
  },
  {
