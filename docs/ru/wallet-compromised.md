...
По той же причине стоит выбрать MEV-защищённый RPC: он снижает шанс, что атакующий увидит вашу транзакцию в общей очереди и опередит вывод. Узнать такую сеть можно по метке **MEV PROTECTED** рядом с её названием в селекторе сети — она видна и на свёрнутом селекторе, и в раскрытом списке.
:::
...
Именно поэтому в AntiDrain есть [кошелёк-донор](/ru/donor-wallet): газ платит отдельный чистый кошелёк, к которому у атакующего доступа нет. Пополнять нужно только его, и только на сумму, которую покажет шаг «Пополнение донора».
:::
...
Такое бывает, и это стоит сказать прямо: отправленную транзакцию в блокчейне отменить нельзя. Если активы уже выведены, вернуть их через AntiDrain нельзя — сайт готовит и отправляет транзакции, но не отменяет чужие.
