...
You do not choose a deadline. Until you sign, the **Permit deadline** field shows "Set automatically when you sign": signing sets how long the signature stays valid, and the field then shows the time it expires. The window is short on purpose, so sign when you are ready to go through the remaining steps straight away.

The site watches that time for you and stops before an expiring signature can be sent. The cure is always the same — sign again in this panel and build again — because nothing later in the process can extend a signature. Where you see it depends on how far you got:

- **Before you build.** Shortly before the deadline, the **Build Transaction** button disables itself, even if the panel is just sitting open, and its explanation says "This permit expires too soon to fund the donor and send the rescue before it does. Sign again to get a fresh one." Once the deadline has passed it says "The permit deadline has passed. Sign again to get a fresh one — an expired permit only spends gas." Press **Generate & sign permitData** again, then build.
- **After you build.** A plan built from permits never outlives them: it expires, at the latest, shortly before the earliest of its signatures does. When the signatures no longer leave enough time, TX Simulator shows a notice saying they expire too soon and disables **Run Simulation**, **Proceed to Fund Donor** and **Continue Anyway**. Once the plan has expired for that reason, Fund Donor says "The Permit signatures this plan relies on expire too soon to fund the donor and send it. Planning again will not help: go back to TX Builder, sign the permits again and approve the new plan." and its **Return to TX Builder and sign again** button takes you there. TX Sender says the plan can no longer be sent and does not send it.
- **While sending.** A rescue can take two transactions: first deploying the rescue contract, then the rescue itself. The site checks the time again before each one and does not send a step the signatures could no longer get through; the log shows which steps already went through. Press **Retry — Back to TX Builder**, sign again and build a new plan. If the contract was already deployed, the new plan first checks whether it can be reused, and if it can, you do not pay for it again. In the rare case a signature expires while a step is on its way, the rescue contract refuses that step: it moves no tokens, and only its network fee is spent.
