...
Nothing was run and nothing came back. The site could not reach the network it checks through, or what came back was not something it could read, or the request never went out — or the plan you built produced no transaction to check in the first place. Whichever it was, your transaction has not been checked yet: nothing here has found anything wrong with it.

**The card's own sentence says which of those it was, and that is what decides your next move.** Where it is about the network — nothing came back, or what came back was not usable — the connection is what to work on, and the rest of this section is for you. Where it sends you back to the **TX Builder** instead, to look again at the transaction or at what the plan selected, pressing the check again cannot help: it never got a transaction to run, and the plan is what has to change.
